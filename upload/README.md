...
When a form event provides only filenames, photo-upload-files reads this site's boatPhotoUpload submissions through the Netlify API. It requires NETLIFY_FORMS_BACKFILL_TOKEN (or the existing aliases NETLIFY_API_TOKEN, NETLIFY_AUTH_TOKEN, NETLIFY_TOKEN) in the Functions environment. SITE_ID is supplied by Netlify; NETLIFY_SITE_ID can override it. Use a token authorized for this site, stored only as a secret, never in source or chat. Redeploy after changing variables.

The lookup matches upload UUID, name, phone, boat details, count and reference-prefixed filenames. It supports plain URL strings, file objects with a url property, and serialized file objects. It follows pagination within this site's upload form, capped at 500 recent submissions. It does not resend historical submissions. Configuration/API/no-match errors emit PHOTO_LOOKUP_* codes without credentials or donor data; a donor-details email still sends with the missing-photo notice. Run node --test tests/photo-upload-email.test.mjs tests/photo-upload-files.test.mjs. Live attachments still require a test with the runtime token configured.
